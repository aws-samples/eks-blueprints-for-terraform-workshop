...
Many Kubernetes addons require authenticated communication with AWS APIs to seamlessly integrate with AWS infrastructure and services. We must configure proper IAM roles and policies to grant these addons the necessary permissions. For example, the AWS Load Balancer Controller addon needs to interact with EC2 APIs to provision Network Load Balancers (NLBs) and Application Load Balancers (ALBs). Similarly, Karpenter needs to interact with EC2 APIs to dynamically provision and terminate compute resources based on cluster needs.
...
This means that we can easily activate these addons through GitOps Bridge afterwards.

### 2. Provide addon IAM role to Argo CD

We use the Terraform EKS Blueprints addons module to create AWS resources for EKS addons. We need to provide these resource identifiers to Argo CD, which handles actually installing the addons on the Kubernetes cluster. In this case, Argo CD will set the IAM roles for the load balancer controller on the addon's service accounts.
